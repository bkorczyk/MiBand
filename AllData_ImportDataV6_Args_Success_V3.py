...
class MiBand5Stable:
    ...
    async def fetch_control_handler(self, sender, data):
        ...
        if data == b'\x10\x01\x01':
        ...
        elif data == b'\x10\x01\x02':
            print("-> Błąd fetch: Nieprawidłowy parametr (np. timestamp).")
            self.fetch_done.set()  # Kończymy, bo błąd
            ...
            print("-> Fetch nieudany (błąd ogólny).")
            self.fetch_done.set()

        else:
            print("-> Nieznany kod odpowiedzi fetch.")
    ...
    async def activity_data_handler(self, sender, data):
        ...
        if len(data) != 20:
            print("-> Nieprawidłowa długość pakietu aktywności - pomijam.")
            return
        ...
        hour = data[5]
        minute = data[6]
        second = data[7]  # Może być 0, jeśli nie wysłane
    ...
    async def run(self):
        ...
        try:
            async with BleakClient(self.mac) as client:
                ...
                # Pobieranie danych bieżących - w try-except
                try:
                    ...
                    print(f"Błąd odczytu bieżących danych: {e}")
                    steps, meters, calories = None, None, None

                # Poziom baterii - w try-except
                try:
                ...
                try:
                    ...
                    await asyncio.sleep(12)  # Wydłużony czas na pomiar
                    await client.stop_notify(UUID_HEART_RATE_MEASUREMENT)
                except Exception as e:
                    print(f"Błąd pomiaru tętna: {e}")
                    self.current_hr = None
                ...
                try:
                    ...
                    time_bytes = bytes([
                        ...
                        start_time.hour, start_time.minute  # Bez sekundy!
                    ])
                    ...
                    await asyncio.sleep(2)  # Czas na pierwszą odpowiedź
                    await asyncio.wait_for(self.fetch_done.wait(), timeout=120)
                except Exception as e:
                    print(f"Błąd fetch historycznych danych: {e}")
